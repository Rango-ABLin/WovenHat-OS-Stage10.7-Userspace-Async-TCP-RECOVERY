...
fn waiting_consumer() -> ! {
    let ok = (|| {
        ...
        loop {
            ...
            if count == 1 {
                if events[0].cookie != 123 || events[0].value != 456 {
                    return None;
                }
...
fn wake_probe() -> bool {
    ...
    if unsafe { task::spawn_on(0, "port-waiter", waiting_consumer) }.is_err() {
        return false;
    }
    let deadline = timer::ticks() + 500;
    while WAIT_HANDLE.load(Ordering::Acquire) == 0 {
        if timer::ticks() >= deadline || WAIT_RESULT.load(Ordering::Acquire) != 0 {
            return false;
        }
        x86_64::instructions::hlt();
    }
    // SAFETY: Producer follows the restricted cross-CPU contract above.
    if unsafe { task::spawn_on(smp::online_count() - 1, "port-waker", waking_producer) }.is_err() {
        return false;
    }
...
fn producer() -> ! {
    ...
    let ok = async_op::Handle::from_raw(raw)
        .is_some_and(|h| async_op::complete(h, async_op::Completion::new(0, cpu as u64)).is_ok());
    if !ok {
        FAILED.fetch_add(1, Ordering::Relaxed);
    }
...
pub fn ports_smp() -> bool {
    ...
    let Ok(port) = completion_port::create(owner) else {
        return false;
    };
    for (cpu, target) in HANDLES.iter().enumerate().take(smp::online_count()) {
        let Ok(handle) = async_op::allocate_current(async_op::AsyncClass::Device) else {
            return false;
        };
        if async_op::associate_current(handle, port, cpu as u64).is_err() {
            return false;
        }
        ...
        if unsafe { task::spawn_on(cpu, "port-producer", producer) }.is_err() {
            return false;
        }
    }
    ...
    let Ok((events, count)) =
        completion_port::begin(owner, port, task, completion_queue::BATCH, false)
    else {
        return false;
    };
    if count != smp::online_count() || FAILED.load(Ordering::Relaxed) != 0 {
        return false;
    }
    let mut mask = 0u64;
    for event in &events[..count] {
        if event.value >= smp::online_count() as u64 || event.cookie != event.value {
            return false;
        }
        mask |= 1 << event.value;
        let Some(handle) = async_op::Handle::from_raw(event.operation) else {
            return false;
        };
        if async_op::release_current(handle).is_err() {
            return false;
        }
    }
