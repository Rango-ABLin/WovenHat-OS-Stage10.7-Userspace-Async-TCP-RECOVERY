...
impl Queue {
    const fn new() -> Self {
        Self {
            entries: [const { None }; MAX_ASYNC_FILE_REQUESTS],
            next_id: 1,
        }
    }
...
fn submit(
    ...
    let (file, scope, under_mnt) =
        task::pin_current_file_for_async(descriptor, write).map_err(|_| ())?;
...
pub fn release_owner(owner: TaskId) -> usize {
    let n = QUEUE.lock().release_owner(owner);
    OWNER_REAPED.fetch_add(n as u64, Ordering::Relaxed);
    n
}
#[cfg(feature = "stage10-5-test")]
pub fn stats() -> Stats {
...
fn process_one() -> bool {
    ...
    let result = match work.request.operation {
        Operation::Read => vfs::read_at(
            work.request.file,
            work.request.offset,
            &mut data[..work.request.length],
        ),
        Operation::Write => vfs::write_at(
            work.request.file,
            work.request.offset,
            &data[..work.request.length],
        ),
    };
    if result.is_ok() && work.request.operation == Operation::Write && work.request.under_mnt {
