...
impl Device for WifiSmolDevice<'_> {
    ...
    fn transmit(&mut self, _timestamp: Instant) -> Option<Self::TxToken<'_>> {
        self.session
            .secure_transport_ready(self.epoch)
            .then_some(WifiTxToken {
                session: self.session,
                epoch: self.epoch,
            })
    }
...
pub fn self_test() -> bool {
    ...
    // Ethernet frame and must push a protected 802.11 frame into the backend.
    {
        ...
        let Some(tx) = dev.transmit(Instant::from_millis(0)) else {
            return false;
        };
        tx.consume(64, |frame| {
    ...
    let Ok(Some(n)) = session.dequeue_tx(&mut protected) else {
        return false;
    };
    if n <= 64 {
        return false;
    ...
    if dev.transmit(Instant::from_millis(1)).is_some() {
        return false;
    }
...
pub fn classify_session_error(error: &SessionError) -> bool {
    matches!(
        error,
        SessionError::StaleEpoch | SessionError::WrongState | SessionError::NoKeys
    )
}
...
pub fn transport_selector_self_test() -> bool {
    ...
    let tk = [0x77; 16];
    let mut session = WifiSession::new(2);
    let Ok(epoch) = session.begin_connect(10, 10) else {
        return false;
    };
    if session
        .install_pairwise_for_test(epoch, station, ap, &tk)
    ...
    let Some(tx) = transport.transmit(Instant::from_millis(0)) else {
        return false;
    };
    tx.consume(64, |frame| {
        frame[..6].copy_from_slice(&peer);
    ...
    let NetTransport::Wifi(device) = &mut transport else {
        return false;
    };
    let mut protected = [0u8; MAX_80211_FRAME];
    let Ok(Some(n)) = device.session_mut().dequeue_tx(&mut protected) else {
        return false;
    };
    if n <= 64 {
        return false;
