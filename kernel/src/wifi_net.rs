...
impl WifiNetBridge {
    pub fn new(station:[u8;6],bssid:[u8;6],tk:&[u8])->Result<Self,BridgeError>{
        Ok(Self{station,bssid,pairwise:TemporalKey::new(tk)?,tx:TxState::new(),rx:RxState::new()})
    }
    ...
    pub fn decapsulate(&mut self,frame:&[u8],out:&mut[u8])->Result<usize,BridgeError>{
        ...
        if h[4..10]!=self.station||h[10..16]!=self.bssid{return Err(BridgeError::WrongPeer)}
        let mut plain=[0u8;MAX_ETH_FRAME-ETH_HEADER_LEN+LLC_SNAP_LEN];
        let len=match wifi_ccmp::unprotect(&self.pairwise,&mut self.rx,frame,&mut plain){
            Ok(n)=>n,Err(e)=>{plain.zeroize();return Err(BridgeError::Ccmp(e));}
        };
