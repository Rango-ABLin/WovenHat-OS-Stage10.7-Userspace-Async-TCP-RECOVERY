...
struct Queue {
    entries: [Option<Notification>; CAPACITY],
    head: usize,
    tail: usize,
    len: usize,
}
...
pub fn publish(note: Notification) -> bool {
    ...
    let tail = q.tail;
    q.entries[tail] = Some(note);
    q.tail = (tail + 1) % CAPACITY;
    q.len += 1;
    true
}
#[allow(dead_code)]
pub fn receive() -> Option<Notification> {
    let mut q = QUEUE.lock();
    let head = q.head;
    let note = q.entries[head].take()?;
    q.head = (head + 1) % CAPACITY;
    q.len -= 1;
    Some(note)
}
    ...
    while offset < q.len {
        ...
        if q.entries[index].is_some_and(|n| n.recipient == recipient) {
            ...
            while i + 1 < q.len {
                ...
                q.entries[to] = q.entries[from].take();
                i += 1;
            }
            q.tail = (q.tail + CAPACITY - 1) % CAPACITY;
            let tail = q.tail;
            q.entries[tail] = None;
            q.len -= 1;
            return note;
...
pub fn structural_self_test() -> bool {
    ...
    publish(note)
        && receive_for(8).is_none()
        && receive_for(9) == Some(note)
        && receive().is_none()
        && publish(note)
        ...
        && discard_recipient(9) == 1
        && receive_for(10).is_some()
        && receive().is_none()
}
