...
impl From<RecoveryError> for SessionError {
    fn from(v: RecoveryError) -> Self {
        Self::Recovery(v)
    }
}
impl From<BackendError> for SessionError {
    fn from(v: BackendError) -> Self {
        Self::Backend(v)
    }
}
...
impl WifiSession {
    pub const fn new(max_attempts: u8) -> Self {
        Self {
            recovery: RecoveryController::new(max_attempts),
            backend: VirtualBackend::new(),
            bridge: None,
            active_epoch: None,
        }
    }
    ...
    pub(crate) fn install_pairwise_for_test(
        &mut self,
        epoch: u32,
        station: [u8; 6],
        bssid: [u8; 6],
        ...
        if self.recovery.state() != RecoveryState::Connecting {
    ...
    pub fn activate_from_wpa2(
        ...
        if !supplicant.gtk_installed() {
            return Err(SessionError::MissingGtk);
        }
        let tk = supplicant.temporal_key().ok_or(SessionError::NoKeys)?;
        let (gtk_index, gtk) = supplicant
            .group_temporal_key()
            .ok_or(SessionError::MissingGtk)?;
    ...
    pub fn refresh_group_key_from_wpa2(
        &mut self,
        epoch: u32,
        supplicant: &Wpa2Supplicant,
    ) -> Result<(), SessionError> {
    ...
    pub fn receive_ethernet(
        ...
        self.require_active(epoch)?;
        let mut frame = [0u8; MAX_80211_FRAME];
        let Some(n) = self.backend.receive_into(&mut frame)? else {
            return Ok(None);
        };
        let bridge = self.bridge.as_mut().ok_or(SessionError::NoKeys)?;
    ...
    pub fn poll_timeout(&mut self, now: u64) -> bool {
        if self.recovery.poll_timeout(now) {
            self.clear_association();
            true
        } else {
            false
    ...
    pub const fn has_pairwise_keys(&self) -> bool {
        self.bridge.is_some()
    }
    ...
    fn require_active(&self, epoch: u32) -> Result<(), SessionError> {
        if self.active_epoch != Some(epoch) || self.recovery.epoch() != epoch {
            return Err(SessionError::StaleEpoch);
        }
        if self.recovery.state() != RecoveryState::Connected {
            return Err(SessionError::WrongState);
        }
        Ok(())
    }
...
pub fn self_test() -> bool {
    ...
    let Ok(e1) = s.begin_connect(10, 5) else {
        return false;
    };
    if s.install_pairwise_for_test(e1, sta, ap, &[0x11; 16])
        .is_err()
        || !s.has_pairwise_keys()
    {
        return false;
    }
    if s.transmit_ethernet(e1, &eth).is_err() {
        return false;
    }
    let mut out = [0u8; MAX_80211_FRAME];
    if !matches!(s.dequeue_tx(&mut out), Ok(Some(_))) {
        return false;
    }
    ...
    if e2 == e1
        || s.has_pairwise_keys()
        || s.transmit_ethernet(e1, &eth) != Err(SessionError::StaleEpoch)
    {
        return false;
    }

    let Ok(e3) = s.begin_connect(20, 2) else {
        return false;
    };
    if e3 != e2 || !s.poll_timeout(22) || s.has_pairwise_keys() {
        return false;
    }
    if s.install_pairwise_for_test(e3, sta, ap, &[0x22; 16]) != Err(SessionError::StaleEpoch) {
        return false;
    }
    true
}
...
pub fn wpa2_handoff_self_test() -> bool {
    ...
    let Ok(epoch) = session.begin_connect(100, 20) else {
        return false;
    };
    ...
    if session.activate_from_wpa2(epoch, station, ap, &supplicant)
    ...
    let Some(n1) = build(&mut msg1, PAIRWISE | ACK | DESC, 1, anonce, &[]) else {
        return false;
    };
    ...
    let Some(n3) = build(
        &mut msg3,
        PAIRWISE | INSTALL | ACK | MIC | SECURE | ENCRYPTED | DESC,
        2,
        anonce,
        &wrapped[..wn],
    ) else {
    ...
    let Ok(mic) = wifi_crypto::compute_eapol_mic(ap_ptk.kck(), &msg3[..n3]) else {
        return false;
    };
    ...
    if supplicant
        .receive_message3_build_message4(&msg3[..n3], &mut msg4)
        .is_err()
    {
        return false;
    }
    if supplicant.gtk_index() != Some(1) || supplicant.gtk_install_count() != 1 {
        return false;
    }

    if session
        .activate_from_wpa2(epoch, station, ap, &supplicant)
        .is_err()
    {
        return false;
    }
    if !session.has_pairwise_keys() {
        return false;
    }
    ...
    if session.transmit_ethernet(epoch, &eth).is_err() {
        return false;
    }
...
pub fn rx_path_self_test() -> bool {
    ...
    let Ok(epoch) = session.begin_connect(1, 10) else {
        return false;
    };
    if session
        .install_pairwise_for_test(epoch, station, ap, &tk)
        .is_err()
    {
        return false;
    }
    ...
    for (i, b) in ethernet[14..].iter_mut().enumerate() {
        *b = i as u8
    }

    let Ok(key) = TemporalKey::new(&tk) else {
        return false;
    };
    let mut ap_tx = TxState::new();

    let mut frame = [0u8; MAX_80211_FRAME];
    let Some(n) = make_downlink(&key, &mut ap_tx, station, ap, peer, &ethernet, &mut frame) else {
        return false;
    };
    if session.inject_rx_for_test(&frame[..n]).is_err() {
        return false;
    }

    let mut recovered = [0u8; 1514];
    let Ok(Some(rn)) = session.receive_ethernet(epoch, &mut recovered) else {
        return false;
    };
    if recovered[..rn] != ethernet {
        return false;
    }

    // Backend queue is empty after consuming the accepted frame.
    if session.receive_ethernet(epoch, &mut recovered) != Ok(None) {
        return false;
    }
    ...
    if session.receive_ethernet(epoch, &mut recovered)
        != Err(SessionError::Bridge(BridgeError::Ccmp(
            wifi_ccmp::CcmpError::Replay,
        )))
    {
        return false;
    }
    ...
    let Some(n2) = make_downlink(&key, &mut ap_tx, station, ap, peer, &ethernet, &mut frame) else {
        return false;
    };
    let mut tampered = frame;
    tampered[n2 - 1] ^= 1;
    if session.inject_rx_for_test(&tampered[..n2]).is_err() {
        return false;
    }
    if session.receive_ethernet(epoch, &mut recovered)
        != Err(SessionError::Bridge(BridgeError::Ccmp(
            wifi_ccmp::CcmpError::Authentication,
        )))
    {
        return false;
    }
    if session.inject_rx_for_test(&frame[..n2]).is_err() {
        return false;
    }
    let Ok(Some(rn2)) = session.receive_ethernet(epoch, &mut recovered) else {
        return false;
    };
    if recovered[..rn2] != ethernet {
        return false;
    }
