...
impl From<EapolError> for SecurityError {
    fn from(v: EapolError) -> Self {
        Self::Protocol(v)
    }
}
impl From<GtkError> for SecurityError {
    fn from(v: GtkError) -> Self {
        Self::Gtk(v)
    }
}
...
impl SecurityContext {
    ...
    pub fn process_verified_message3(
        &mut self,
        message3: &[u8],
    ) -> Result<InstallOutcome, SecurityError> {
        let key = wifi_rsn::parse_eapol_key(message3)?;
        let Some(ptk) = self.ptk.as_ref() else {
            return Err(SecurityError::NoPairwiseKey);
        };
        if key.nonce != self.anonce {
            return Err(SecurityError::WrongNonce);
        }
        if let Some(last) = self.replay {
            if key.replay_counter < last {
                return Err(SecurityError::WrongReplay);
            }
        }
        if !key.key_info.encrypted_key_data() || key.key_data.is_empty() {
            return Err(SecurityError::MissingEncryptedKeyData);
        }
    ...
    pub fn install_verified_gtk_for_test(
        &mut self,
        replay: u64,
        index: u8,
        key: [u8; wifi_gtk::GTK_LEN],
    ) -> Result<InstallOutcome, SecurityError> {
...
pub fn self_test() -> bool {
    ...
    if ctx.temporal_key() != Some(&[0x44; 16][..]) {
        return false;
    }
    let gtk = [0x55; wifi_gtk::GTK_LEN];
    if ctx.install_verified_gtk_for_test(7, 1, gtk) != Ok(InstallOutcome::Installed) {
        return false;
    }
    if ctx.install_verified_gtk_for_test(7, 1, gtk) != Ok(InstallOutcome::Retransmission) {
        return false;
    }
    if ctx.gtk_install_count() != 1 || !ctx.gtk_installed() {
        return false;
    }
    if ctx.install_verified_gtk_for_test(7, 1, [0x56; wifi_gtk::GTK_LEN])
        != Err(SecurityError::Gtk(GtkError::ConflictingRetransmission))
    {
        return false;
    }
    ctx.clear();
