...
use crate::{
    completion_queue::{BATCH, Event, Queue},
    irq_lock::IrqMutex,
    task::{self, TaskId},
};
...
impl Port {
    const fn new() -> Self {
        Self {
            ...
            owner: None,
            queue: Queue::new(),
            waiters: [None; crate::config::MAX_TASKS],
            consumer: None,
        }
    }
    fn matches(&self, handle: u64) -> bool {
        self.owner.is_some() && self.generation == (handle >> 32) as u32
    }
    fn clear(&mut self) {
        self.owner = None;
...
static TABLE: [IrqMutex<Port>; PORTS] = [const { IrqMutex::with_rank(Port::new(), 20) }; PORTS];

fn index(handle: u64) -> Result<usize, ()> {
    let index = (handle & 0xffff) as usize;
    if handle & 0xffff_0000 != TAG || handle >> 32 == 0 || index >= PORTS {
        return Err(());
    }
    Ok(index)
}
...
pub fn create(owner: u64) -> Result<u64, ()> {
    ...
    if TABLE
        .iter()
        .filter(|p| p.lock().owner == Some(owner))
        .count()
        >= PER_PROCESS
    {
        return Err(());
...
pub fn close(owner: u64, handle: u64) -> Result<(), ()> {
    let wake = {
        ...
        if !port.matches(handle) || port.owner != Some(owner) {
            return Err(());
        }
...
pub fn reserve(owner: u64, port: u64, operation: u64, cookie: u64) -> Result<(), ()> {
    ...
    if !slot.matches(port) || slot.owner != Some(owner) || !slot.queue.reserve(operation, cookie) {
        return Err(());
    }
    Ok(())
}
...
pub fn publish(
    ...
    let Ok(index) = index(port) else {
        return [None; crate::config::MAX_TASKS];
    };
    let mut slot = TABLE[index].lock();
    if !slot.matches(port) || !slot.queue.publish(operation, class, flags, status, value) {
        return [None; crate::config::MAX_TASKS];
    }
...
pub fn abandon(port: u64, operation: u64) {
    ...
    let mut slot = TABLE[index].lock();
    if slot.matches(port) {
        slot.queue.remove(operation);
    }
}

pub fn notify(waiters: Waiters) {
    for waiter in waiters.into_iter().flatten() {
        task::signal_event(waiter);
    }
}
...
pub fn begin(
    ...
    capacity: usize,
    wait: bool,
) -> Result<([Event; BATCH], usize), ()> {
    if capacity == 0 || capacity > BATCH {
        return Err(());
    }
    let mut port = TABLE[index(handle)?].lock();
    if !port.matches(handle) || port.owner != Some(owner) || port.consumer.is_some() {
        return Err(());
    }
    ...
    if count != 0 {
        ...
        let Some(slot) = port.waiters.iter_mut().find(|s| s.is_none()) else {
            return Err(());
        };
        *slot = Some(task);
    }
...
pub fn finish(
    ...
    let mut port = TABLE[index(handle)?].lock();
    if !port.matches(handle) || port.owner != Some(owner) || port.consumer != Some(task) {
        return Err(());
    }
    if copied {
        for event in events {
            port.queue.remove(event.operation);
        }
    }
    port.consumer = None;
    Ok(())
...
pub fn unwatch(handle: u64, task: TaskId) {
    let Ok(index) = index(handle) else {
        return;
    };
    let mut port = TABLE[index].lock();
    if port.matches(handle) {
        for waiter in &mut port.waiters {
            if *waiter == Some(task) {
                *waiter = None;
            }
        }
