...
fn constant_time_eq(left: &[u8], right: &[u8]) -> bool {
    ...
    for index in 0..left.len() { difference |= left[index] ^ right[index]; }
    difference == 0
}
...
#[inline]
fn store32(out: &mut [u8], value: u32) { out[..4].copy_from_slice(&value.to_le_bytes()); }
...
fn chacha_block(key: &[u8; KEY_SIZE], nonce: &[u8; 12], counter: u32) -> [u8; 64] {
    ...
    for index in 0..8 { state[4 + index] = load32(&key[index * 4..]); }
    state[12] = counter;
    for index in 0..3 { state[13 + index] = load32(&nonce[index * 4..]); }
    let original = state;
    for _ in 0..10 {
    ...
    for index in 0..16 { store32(&mut block[index * 4..], state[index].wrapping_add(original[index])); }
    block
}
...
fn xor_stream(key: &[u8; KEY_SIZE], nonce: &[u8; 12], counter: u32, data: &mut [u8]) {
    for (block_index, chunk) in data.chunks_mut(64).enumerate() {
        ...
        for (dst, src) in chunk.iter_mut().zip(stream) { *dst ^= src; }
    }
}

fn authentication_tag(key: &[u8; KEY_SIZE], nonce: &[u8; 12], aad: &[u8], ciphertext: &[u8]) -> Tag {
...
pub fn open(key: &[u8; KEY_SIZE], nonce: u64, aad: &[u8], data: &mut [u8], tag: &Tag) -> bool {
    ...
    if !constant_time_eq(&expected, tag) { return false; }
    xor_stream(key, &nonce, 1, data);
    true
}
...
pub fn structural_self_test() -> bool {
    ...
    let rfc_nonce = [0x07, 0x00, 0x00, 0x00, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47];
    let rfc_aad = [0x50, 0x51, 0x52, 0x53, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7];
    ...
    if rfc_tag != [0x1a, 0xe1, 0x0b, 0x59, 0x4f, 0x09, 0xe2, 0x6a, 0x7e, 0x90, 0x2e, 0xcb, 0xd0, 0x60, 0x06, 0x91] { return false; }

    let key = [0x42u8; KEY_SIZE];
    let mut data = *b"volume metadata and payload";
    let original = data;
    let Some(tag) = seal(&key, 9, b"header", &mut data) else { return false; };
    if data == original { return false; }
    let mut wrong_tag = tag;
    wrong_tag[0] ^= 1;
    if open(&key, 9, b"header", &mut data, &wrong_tag) { return false; }
