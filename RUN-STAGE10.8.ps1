...
try {
    ...
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "run-stage10-8-acceptance.ps1") 2>&1 |
        ForEach-Object { $_.ToString() } |
        Tee-Object -FilePath (Join-Path $auditRoot 'acceptance-output.txt') | Out-Host
    $gateExitCode = $LASTEXITCODE
    $ErrorActionPreference = 'Stop'
    if ($gateExitCode -ne 0) { throw "Stage 10.8 acceptance failed with exit code $gateExitCode" }
}
finally {
