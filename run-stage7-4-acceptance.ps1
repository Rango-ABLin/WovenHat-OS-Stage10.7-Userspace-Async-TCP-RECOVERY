...
Write-Host '=== WovenHat Stage 7.4 Ready-state Ring-3 Migration & Affinity Acceptance ==='

$artifactRoot = '.\stage7-4-artifacts'
...
function Save-Failure([int]$Cpu, [int]$Run, [string]$Suite) {
    ...
    if (Test-Path $serial) {
        ...
        Select-String -Path $serial -Pattern 'S7.3|S7.4|TERM|panic|assertion|page fault|#PF|exception|scheduler|Switching|Blocked|Sleeping|Ready|Running|terminated' |
...
function Run-MemoryStress([int]$Cpu, [int]$Runs) {
    ...
    for ($run = 1; $run -le $Runs; $run++) {
        ...
        if ($code -ne 0) {
            ...
            throw "Stage 7.4 memory test failed on $Cpu CPU(s), run $run"
        }
        if (-not (Select-String -Path $src -SimpleMatch '[TERM] scheduler-owned Ready-task termination + deferred reap: PASSED' -Quiet)) {
            ...
            throw "Stage 7.4 regression: Stage 7.2 termination marker missing on $Cpu CPU(s), run $run"
        }
        if ($Cpu -eq 1) {
            if (-not (Select-String -Path $src -SimpleMatch '[S7.3] single-CPU baseline preserved; no AP Ring-3 probe required' -Quiet)) {
                ...
                throw "Stage 7.4 regression: Stage 7.3 single-CPU preservation marker missing on run $run"
            }
        }
        else {
            if (-not (Select-String -Path $src -SimpleMatch '[S7.3] pinned Ring-3 execution on every AP: PASSED' -Quiet)) {
                Save-Failure $Cpu $run 'stage7-3-marker'
                throw "Stage 7.4 regression: Stage 7.3 AP Ring-3 marker missing on $Cpu CPU(s), run $run"
            }
        }
        if ($Cpu -eq 1) {
            if (-not (Select-String -Path $src -SimpleMatch '[S7.4] single-CPU baseline preserved; no Ring-3 migration required' -Quiet)) {
                ...
                throw "Stage 7.4 single-CPU preservation marker missing on run $run"
            }
        }
        else {
            if (-not (Select-String -Path $src -SimpleMatch '[S7.4] Ready-state Ring-3 migration + affinity: PASSED' -Quiet)) {
                Save-Failure $Cpu $run 'stage7-4-marker'
                throw "Stage 7.4 migration/affinity marker missing on $Cpu CPU(s), run $run"
            }
        }
    }
}
...
function Run-Network([int]$Cpu) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Stage 7.4 network regression failed on $Cpu CPU(s)" }
}
...
# Stage 7.3 is the validated baseline. Stage 7.4 changes only Ready-state
# ownership/affinity for the narrow migratable Ring-3 probe class. Preserve all
# 7.2 lifecycle, 7.3 AP Ring-3, build/lint/test and network regression gates.
Write-Host "`n=== 4/6 Stage 7.3 preservation + 1-CPU regression: 10 runs ==="
Run-MemoryStress 1 10
Write-Host "`n=== 5/6 Stage 7.4 Ready Ring-3 migration/affinity: 2 CPU x20, 4 CPU x10 ==="
...
Write-Host "`n=== STAGE 7.4 ACCEPTANCE: PASS ==="
Write-Host 'Stage 7.3 AP Ring-3 guarantees preserved; Ready-state Ring-3 migration, hard affinity and network regression passed.'
