...
The FAT32 layer now has a bounded, checksum-correct UTF-8 LFN record encoder
and decoder for standard 13-character UTF-16 directory slots. It validates
path-safe input, handles surrogate pairs, emits reverse ordinal records, and
resolves existing LFN names through path lookup. The codec and lookup paths are
covered by the Stage 1–5 structural gate on 1/2/4 CPUs.
...
and rolls those links back on failed publication. Unicode names remain bounded
by the existing path and VFS entry limits; normalization and a larger Unicode
path ABI are separate work.
