...
| --- | --- | --- |
| Stages 1–5 | Boot, memory, paging, scheduler, VFS, userspace, storage; 1/2/4-CPU acceptance; bounded long-filename create/lookup/delete and mounted import listing | Non-QEMU hardware qualification/drivers, DMA storage completion, long-filename rename, low-level list-name field, Unicode and directory extension, crash-safe metadata ordering, per-file permissions, unclean-shutdown recovery |
