...
| --- | --- | --- |
| Stages 1–5 | Boot, memory, paging, scheduler, VFS, userspace, storage; 1/2/4-CPU acceptance; bounded Unicode long-filename create/lookup/delete/rename, named listing, mounted import listing, directory growth, bounded VFS uid/gid/mode enforcement, bounded FAT32 ownership metadata persistence/import and file lifecycle cleanup, and pending-intent recovery after interrupted metadata updates | Non-QEMU hardware qualification/drivers, DMA storage completion, Unicode normalization/larger path ABI, collision-resistant/full-capacity metadata identity, directory subtree metadata transactions, full durable multi-operation journal replay, unclean-shutdown hardware qualification |
