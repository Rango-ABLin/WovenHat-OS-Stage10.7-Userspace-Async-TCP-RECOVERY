...
| --- | --- | --- |
| Stages 1–5 | Boot, memory, paging, scheduler, VFS, userspace, storage; 1/2/4-CPU acceptance; bounded long-filename create/lookup/delete/rename, mounted import listing, and directory growth | Non-QEMU hardware qualification/drivers, DMA storage completion, low-level list-name field, Unicode, crash-safe metadata ordering, per-file permissions, unclean-shutdown recovery |
