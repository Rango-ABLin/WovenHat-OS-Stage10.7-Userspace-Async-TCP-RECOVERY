...
| Stage 6 | Bounded SMP, TLB shootdowns, 1–4 CPU tests, ACPI SRAT CPU/memory-domain discovery, domain-local placement/rebalancing/allocation, x2APIC MSR path, migratable audited I/O workers, bounded contiguous-prefix AP offline/re-online control, ranked interrupt-safe locks for scheduler/process/paging/COW/frame-allocation/file-frame cache, async/completion/worker, pipe, IPC, and WovenGuard lineage domains, generation-tagged VFS/pipe handles, allocator-reserved contiguous VirtIO DMA arena | General multicore userspace, unrestricted concurrent I/O/DMA throughput, multi-node NUMA qualification, non-contiguous hotplug hardware qualification, APIC-ID-above-255 hardware coverage, VFS disk-I/O lock separation, remaining compatibility-mutex lock domains/priority inheritance |
